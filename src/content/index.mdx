---
title: 'Rhythm'
tagline: 'Typography-driven spacing.'
---
...
<aside>
	Supported in Chrome 133+ and other browsers with ≥ 86% coverage. [Can I
	Use](https://caniuse.com/mdn-css_properties_text-box)
</aside>
...
Get metrics for your font [from Richard Rutter’s calculator](https://clagnut.com/sandbox/font-size-adjust.html)

</aside>

## Why it works

Most design systems define a <span class="small-caps">module</span>—a base unit, typically 8px, used to scale everything from font sizes to component dimensions. While a fixed module makes layout decisions predictable, spacing is the one area where this approach breaks down.

<aside>
	[Grid](https://guides.kontur.ru/principles/base/grid/)—Kontur Guides (Russian)
</aside>
...
The visual gap between lines is the natural choice for a spacing unit. It’s the primitive designers already look to when balancing <span class="small-caps">the inner and the outer.</span>
...
<aside>
	See: [The rule of the inner and the
	outer](https://bureau.rocks/books/typography-en/6)
</aside>
...
<aside>
	See also: [Avoiding common layout mistakes using visual line
	gap](https://bureau.ru/soviet/20191002/) (Russian)
</aside>
...
Supported families include spacing (`p`, `m`, `gap`), dimensions (`w`, `h`, `min-w`, `max-w`, `min-h`, `max-h`, `size`, `basis`), and physical or logical positioning (`inset`, `top`, `right`, `bottom`, `left`, `start`, `end`). Negative values use UnoCSS's standard `-` variant; the custom rule does not process the prefix itself.
