...
<aside>
	Chrome 133+, браузерная поддержка ≥ 86%.
...
Возьмите значения из [калькулятора Ричарда Раттера](https://clagnut.com/sandbox/font-size-adjust.html)
...
Подход опирается на возможности современного CSS: `text-box`, `rlh`, `rex` и `lh`. Браузерная поддержка — ≥ 86%.
...
Правило охватывает отступы (`p`, `m`, `gap`), размеры (`w`, `h`, `min-w`, `max-w`, `min-h`, `max-h`, `size`, `basis`) и физическое или логическое позиционирование (`inset`, `top`, `right`, `bottom`, `left`, `start`, `end`). Отрицательные значения обрабатывает стандартный вариант `-` в UnoCSS; самому правилу разбирать этот префикс не нужно.
...
<pre
	class="p-1g -m-0.5g mbs-2gr gap-x-1g min-w-12g max-w-40g size-4g basis-12g start-1g -top-0.5gr"
></pre>
```
